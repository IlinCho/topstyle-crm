...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !verify_csrf_token($_POST['csrf_token'] ?? '')) {
    ...
    if ($name === '' || $sku === '' || $categoryId === '' || $priceBgn <= 0 || $priceEur <= 0) {
        ...
        try {
            if ($__existing) {
                db_query(
                    'UPDATE product SET name=?, sku=?, slug=?, description=?, material=?, color=?, price_eur=?, price_bgn=?, active=?, category_id=?, category_rank=?, badges=?, size_chart_url=?, size_chart_table=? WHERE id=?',
                    [$name, $sku, $slug, $description, $material, $color, $priceEur, $priceBgn, $active, $categoryId, $categoryRank, $badges, $sizeChartUrl, $sizeChartTable, $__existing['id']]
                );
                $__productId = $__existing['id'];
            } else {
                $__productId = db_id();
                db_query(
                    'INSERT INTO product (id, sku, name, slug, description, material, color, price_eur, price_bgn, active, category_rank, badges, size_chart_url, size_chart_table, category_id)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                    [$__productId, $sku, $name, $slug, $description, $material, $color, $priceEur, $priceBgn, $active, $categoryRank, $badges, $sizeChartUrl, $sizeChartTable, $categoryId]
                );
            }
