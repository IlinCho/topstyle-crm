...
// ps_product.price in PrestaShop is stored WITHOUT tax - the storefront adds
// the tax rule at display time. Bulgaria's standard VAT rate (clothing has
// no reduced rate) is 20%, so the price actually shown to customers on
// topstyle.bg is price * 1.20. Confirmed against a live example: windbreaker
// SKU 2383 is stored as 47.49 (no VAT) but shows as 56.99 on topstyle.bg
// (47.49 * 1.2 = 56.988 -> rounds to 56.99).
const VAT_RATE = 1.2;
...
// Neon's pooled connection occasionally drops mid-script on a long run like
// this (~1000+ products, thousands of round trips) - "Server has closed the
// connection" (Prisma code P1017), or similar transient P1001/P1002 errors.
// It's not a bug in the data, just a flaky connection - retrying the same
// (idempotent) write after a short pause almost always succeeds.
...
async function main() {
  ...
  // Slugs already used by products already in the database (from this or an
  // earlier run) - seeded up front so a brand-new product this run never
  // picks a slug that collides with one that already exists (the previous
  // version of this script only checked collisions against slugs generated
  // *within the same run*, which broke once a re-run started including many
  // more products at once - e.g. inactive ones that were skipped before).
  const dbProducts = await prisma.product.findMany({ select: { sku: true, slug: true } });
  const existingSlugBySku = Object.fromEntries(dbProducts.map((p) => [p.sku, p.slug]));
  ...
  for (const p of products) {
    ...
    const priceEur = Math.round((parseFloat(p.price) || 0) * VAT_RATE * 100) / 100;
    ...
    // --- Live Postgres write (Next.js side) - wrapped in withRetry since
    // this whole block is safe to redo from scratch (upsert-by-sku +
    // delete-then-recreate variants + only-create-image-if-none-yet) if a
    // Neon connection hiccup interrupts it partway through.
